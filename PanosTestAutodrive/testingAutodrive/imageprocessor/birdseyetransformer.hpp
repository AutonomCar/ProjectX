...
namespace Autodrive
{
	/*introduces the new type struct "lanes" and defines its meaning*/
    struct lanes{
        linef left;
        linef right;
        bool found = false;
	...
	/*First arg takes a ref of canniedMat header and guarantees that it will not be changed (The data it points to CAN be changed)
	Second arg */
    lanes getLaneMarkings(const cv::Mat& canniedMat,cv::Mat* drawMat){
        lanes lanes;
        ::std::vector<cv::Vec4i> lines; /*Create a vector "lines", Vec4i= 4 ints, Ex: Vec4i(6,5,4,3);*/
        linef leftMostLine;
        linef rightMostline;
        cv::HoughLinesP(canniedMat, lines, 1, CV_PI / 180, 20, 10, 50);//HoughLinesP gives you the end-points of the lines, so that you can simply get the length as the euclidean distance between these points
        bool foundLeft = false;
        bool foundRight = false;
        int center = canniedMat.size().width / 2; // the center "middle" of the binary image

        static linef lastLML;//Last leftMostLine
        static linef lastRML;//Last rightMostLine
