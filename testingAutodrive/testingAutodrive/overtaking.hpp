...
namespace Autodrive {
    namespace Overtaking {
        ...
        int turnLeft = 0; // För att svänga undan
        int turnLeftCalibration = 0;
        int oomphTurnLeft = 0;
        int turnRight = 0;// svänga tillbaka
		int turnRightCalibration = 0;
        int oomphTurnRight = 0;
        bool turnLeftCalibrationFinished = false;
        int obstacleDistance = 70; // när ska vi börja svänga? hur nära obj kan vi komma?
        ...
        command run(command lastCommand, Mat* mat) {
           /* usFront = SensorData::ultrasound.front;
			...
			//usFrontLeft = SensorData::ultrasound.frontleft;
			irRearRight = SensorData::infrared.rearright;
           /* irDownRight = SensorData::infrared.downRight;
            irDownLeft = SensorData::infrared.downLeft;*/
			
		

            distanceTravelled = SensorData::encoderDistance();// vet inte om behövs atm
            lineLeftFound = SensorData::lineLeftFound;
            lineRightFound = SensorData::lineRightFound;

            lastCommand.setSpeed(0.35);// sänker farten (Vi får justera till våra värden)
            if (stop) lastCommand.setSpeed(0);

            if (usFront > 0 && usFront < obstacleDistance) {// inte krockat och för nära obj
                if (! overtaking) {
                    overtaking = true; // aktivera overtaking MODE
            ...
            if (overtaking) { // obstacle spotted, start turning left
                if (turnLeft) {
                    lastCommand.setAngle(-1);// -1 MAX vänstersväng
